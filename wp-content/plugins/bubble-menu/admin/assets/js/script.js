...
(function($) {
  ...
  $('.item-bgcolor, .item-color').each(function() {
    panelHeadingIconItemColor(this);
  });
  $('body').on('change', '.item-bgcolor, .item-color', function() {
    panelHeadingIconItemColor(this);
  });

  function panelHeadingIconItemColor(el) {
    const colors = {
      white: '#ffffff',
      grey: '#808080',
      black: '#000000',
    ...
    const parent = $(el).parents('.panel');
    const color = $(el).val();
    if ($(el).hasClass('item-bgcolor')) {
      $(parent).find('.icon-select').css('background-color', colors[color]);
    } else if ($(el).hasClass('item-color')) {
      $(parent).find('.icon-select').css('color', colors[color]);
    }
  }
  ...
  iconShapes();
  function iconShapes() {
    const shape = $('#shapes').val();
    switch (shape) {
      case "wow-bmp-shape-rsquare":
        $('.icon-select').css('border-radius', '25%');
        break;
      case "wow-bmp-shape-ellipse":
        $('.icon-select').css('border-radius', '25% 75%');
        break;
      case "wow-bmp-shape-square":
        $('.icon-select').css('border-radius', '0%');
        break;
      default:
        $('.icon-select').css('border-radius', '50%');
    }
  ...
  scrollOption();
  $('#scroll-option').on('change', function() {
    scrollOption();
  });

  function scrollOption() {
    const scroll = $('#scroll-option').val();
    if (scroll === 'show' || scroll === 'hide') {
      $('.scroll-position').removeClass('is-hidden');
    } else {
      $('.scroll-position').addClass('is-hidden');
    }
  }


  $('body').on('change', '#select_icon, #icon_position', function() {
    menuicon();
  });

  function menuicon() {
    var icon = $('#select_icon').val();
    var position = $('#icon_position').val();
    $('#icon_class').text(icon + position);
  }
